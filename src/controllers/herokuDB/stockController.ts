...
const stockController = {
  ...
  async addStockDoc(req: any, res: any) {
    const { storehouseid, documenttype, notesclear, reference, devisLine } =
      req.body;
    const date = new Date();
    ...
    const query =
      'INSERT INTO "StockDocument" (id, storehouseid, documentdate, reference, numberprefix, notesclear) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;';
    ...
    try {
      ...
      await Promise.all(
        devisLine.map(async (line: any) => {
          const { id, caption, quantity } = line;
          const query2 =
            'INSERT INTO "StockDocumentLine" (documentid, descriptionclear, itemid, quantity) VALUES ($1, $2, $3, $4) RETURNING *;';
          ...
          console.log(
            "Stock Document Line added successfully, ",
            result2.rows[0]
          );
          ...
          if (documentCode === "BE" || documentCode === "BR") {
            const updateQuery =
              'UPDATE "Item" SET realstock = realstock + $1 WHERE id = $2';
  ...
  async getStockDocAndLines(req: any, res: any) {
    // ! GET
    try {
  ...
  async getStockDocWithLines(req: any, res: any) {
    // ! POST
    try {
  ...
  async getDocLineWithPrice(req: any, res: any) {

    try {
      const query = `SELECT sdl.*, i.salepricevatincluded
